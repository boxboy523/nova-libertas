    ...
    query
        ...
        .for_each(|(entity, transform, battle_stats)| {
            ...
            let mut nearby_entities = if let Ok(entity_info_vec) =
                spatial_grid.query_entities(transform.position, battle_stats.attack_range, true)
            {
                entity_info_vec
                    .into_iter()
                    .map(|e| e.entity)
            ...
            nearby_entities.sort_by(|&a, &b| {
                let dist_a = query_transform.get(a).map_or(f32::MAX, |t| {
                    transform.position.distance_squared_to(t.position)
                });
    ...
    attackers
        ...
        .for_each(|(entity, mut attack, mut movement, battle_stats)| {
            ...
            let dir = if let Ok(target_transform) = query_transform.get(attack.target) {
                (transform.position - target_transform.position).normalized_or_zero()
            } else {
                Vector2::ZERO
            };
